  ...
  const onSubmit: SubmitHandler<FormData> = (data) => {
    ...
    for (const key in data) {
      if (Object.prototype.hasOwnProperty.call(data, key)) {
        ...
        // Updated to match CartItemOption type: { [key: string]: { values: { [key: string]: { price: number } } } }
        cartItemOptions[key] = {
          values: {
            [optionValue.label]: {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 p-4">
          <>
            {option_keys.map((key) => {
              ...
              const optionValueEntries = Object.entries(currentOption.values);
              return (
                <div key={key}>
                  <Controller
                    control={control}
                        ...
                        <div className="form-control w-full">
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              field.onChange(currentOption.values[selectedKey]);
                            }}
                          >
                            <option disabled value="" className="">
                              Pick your {currentOption.label}
                            </option>
                            {optionValueEntries.map(([valKey, optionValue]) => (
